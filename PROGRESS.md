...
**Highlights — Phase 2 ✓ (CodeMirror edit mode, on `feat/markdown`, pending live verify)** — the note editor is now **CodeMirror 6** (via `@uiw/react-codemirror` + `@codemirror/lang-markdown` GFM base), replacing the plain textarea (`src/views/NoteEditor.tsx`). **Live source styling** (`HighlightStyle` on Lezer tags, theme-aware via CSS vars): headings grow, **bold**/italic/strike read as such, links + mono code stand out, the `#`/`**`/`-` markers dim. **`==highlights==` are decorated live** with the same pastel washes (a `ViewPlugin` + `RangeSetBuilder`; colors shared with preview through `--hl-*` CSS vars — one source of truth for `<mark>` and `.cm-hl`). **Right-click a highlight in the editor → the same recolor palette** (`posAtCoords`→`findHighlightAt`→minimal `view.dispatch`, preserving cursor + undo). Chrome stripped (no fold/active-line) for a prose feel; **compact dimmed line-number gutter**; transparent bg inherits the note font/weight/size + rose caret/selection; **Ctrl-Z undo** comes free with CM history. The recolor palette + swatch chips live in shared `src/views/HighlightPalette.tsx`. **Safety net:** `SafeNoteEditor` wraps the editor in an error boundary that falls back to a plain textarea if CM ever throws. **Verify:** Edit a note → headings/bold/links styled while typing; `==x==` washed yellow live; right-click a highlight → recolor; Ctrl-Z undoes; line numbers are small + faint; switch themes → editor adapts. Then: **drag-tabs-between-panes**. AI stays last.
