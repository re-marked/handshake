...
## ▶ NEXT
**Shell L3 — the workspace, in 5 steps** (full plan: `.claude/plans/curious-stargazing-music.md`). **Steps 1–2 ✓ — verify live.** Main area is a **tab workspace**: everything is a View; browser-style tabs (per-tab icons, person avatars, a sliding rose highlight) + freestyle **`+`** and **split** launchers (shadcn Popover+Command); the rail opens/focuses **singleton** tabs; board is a **pinned home tab**, always-mounted. **Splits** (shadcn Resizable, `react-resizable-panels` **v2** — v4 is a breaking rewrite): ⌘/Ctrl-click a card → person opens beside the board; split-launcher opens any view in a pane; drag dividers to resize; closing a pane's last tab collapses it. Board **remounts-with-restore** on split (state from `layout.json`; BoardSurface deferred unless the blip bugs). Model frozen (`src/workspace/model.ts`). **Verify in `pnpm tauri dev`:** tabs/launchers; ⌘-click-split + drag-resize + close-collapses-pane; board survives; slide-in note (now below the bar) unchanged. Next: **(3)** floating windows, **(4)** inline note-mode switch (panel⇄float⇄tab) + remembered default, **(5)** persist `workspace.json`. Deferred: pathfinding, photo upload, markdown, undo. AI stays last.
...
- [~] App shell — Obsidian-grade workspace (everything is a View; board pinned-main; notes slide in). Model + L0–L6 in **SHELL.md**. [L0 store+frame ✓ · L1 note panel ✓ · faint-**+** create-and-connect ✓ · delete + connection settings (click) ✓ · drag-to-connect ✓ · L2 `Ctrl-P` palette ✓ · primary views Board · People · Goals ✓ · People list: search/sort/density/pagination/locate/add ✓ · **L3 workspace** steps 1–2 ✓ (tab tree + launchers + resizable splits, board pinned) · next: L3 steps 3–5 (floats/note-modes/persist)]
