...
**Next (Mark's picks):** (1) a **Settings page** — the NavRail gear is a stub (`view:"settings"` renders "coming soon"); make it a real view for truly-global prefs (note default, density, layout, vault path, theme…). (2) a **Vault manager** — open / switch / create vaults; today the app binds a single vault at `init(vault)` with no UI to change it. Also still open: **photo upload** (real faces on cards — top pick), markdown rendering in notes, undo (Ctrl-Z). AI stays last.
